...
public class GetBangScores {
    ...
    public static BangHighScores getBangScores(Guild guild){
        ...
        try {
            // Most attempts
            PreparedStatement getMostAttempts = conn.prepareStatement("SELECT tries, user, MAX(tries) FROM bang WHERE "+date.getTime()+" - last_played < 604800000");
            ...
            // Most deaths
            PreparedStatement getMostDeaths = conn.prepareStatement("SELECT tries, user, MAX(deaths) FROM bang WHERE "+date.getTime()+" - last_played < 604800000");
            ...
            // Luckiest
            PreparedStatement getLuckiest = conn.prepareStatement("SELECT tries, user, MIN(death_rate) FROM bang WHERE "+date.getTime()+" - last_played < 604800000");
            ...
            // Unluckiest
            PreparedStatement getUnluckiest = conn.prepareStatement("SELECT tries, user, MAX(death_rate) FROM bang WHERE "+date.getTime()+" - last_played < 604800000");
